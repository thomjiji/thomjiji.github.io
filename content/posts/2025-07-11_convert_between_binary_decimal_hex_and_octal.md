...
### Using Emacs Calc (Base to Decimal)
...
## 使用 M-: (eval-expression)

1.  按 `M-:` (eval-expression)
2.  输入你想要转换的十进制数，比如 192，按 `RET`
3.  得到该十进制数对应的八进制和十六进制结果：`192 (#o300, #xc0)`

可以看到这个方法只给到了 octal 和 hex，并没给到 binary 的结果。当然你可以输入一个 binary，比如 `#b11000000` 然后按 RET 即可得到一样的输出：192 (#o300, #xc0)。所以我们是可以通过这个方法做 binary 到 decimal、octal、hex 的转换，但是！不可以做到 binary 的转换，it's not possible.
